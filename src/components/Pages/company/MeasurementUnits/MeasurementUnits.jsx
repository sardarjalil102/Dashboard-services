...
const MeasurementUnits = () => {
  ...
  const {
    Measurement: { allMeasurements, status, paginationData },
  } = useSelector((state) => state);
  const { permissions } = useSelector((state) => state.Auth);

  useEffect(() => {
    dispatch(fetchAllMeasurement(offset));
  }, [dispatch, offset]);

  useEffect(() => {
    if (isAllow) {
      // console.log("isAllow", isAllow);
      dispatch(deleteMeasurement(getId));
    }
    setIsAllow(false);
  }, [getId, isAllow]);
    ...
    <>
      <div className="row">
        <MainBreadcrum
          ...
          links={[
            { path: "/", title: "" },
            { path: "", title: "Measurement Units", activeLink: true },
          ]}
        />
        {permissions.includes("MeasurementUnits-Create") ? (
          <div className="col-4 align-self-center text-end">
            <Link to="/measurement-units/create">
                ...
                <table className="table">
                  ...
                  <tbody className="table-border-bottom-0 text-center">
                    {allMeasurements.map((m) => (
                      <>
                        <tr>
                          <th scope="row">{m.code}</th>
                          <td>{m.name}</td>
                          {/* <td>{m.unitType}</td> */}
                          <td className="py-0">
                            <div className="d-flex align-items-center justify-content-center">
                              {permissions.includes("MeasurementUnits-View") ? (
                                <Link
                                  className="dropdown-item hover-blue p-3 text-center me-4 rounded pointer"
                                  to={`/measurement-unit/${m.id}`}
                                >
                                  <BsEyeFill size={"1rem"} />
                                </Link>
                              ) : null}
